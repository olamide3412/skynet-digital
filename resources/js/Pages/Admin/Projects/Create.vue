...
const selectedServiceMode = ref('');
const customService = ref('');
const selectedCategoryMode = ref('');
...
// Media gallery state (local preview items)
// Items structure: { id, source: 'file'|'url', type: 'image'|'video', file: File|null, url: String, previewUrl: String }
...
const syncFormMedia = () => {
    form.media_files = galleryItems.value.filter(i => i.source === 'file').map(i => i.file);
    form.media_urls = galleryItems.value.filter(i => i.source === 'url').map(i => ({ type: i.type, url: i.url }));
};
...
<template>
    ...
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 pb-4 border-b border-gray-200 dark:border-gray-800">
            <div>
                <h1 class="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">New Project</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Create a new project showcase with extended details and up to 15 gallery media items.</p>
            </div>
                ...
                <div class="lg:col-span-1 space-y-6">
                    ...
                    <div class="bg-white dark:bg-gray-800/90 border border-gray-200 dark:border-gray-700/80 rounded-2xl p-6 shadow-sm">
                        ...
                        >
                            ...
                            <span>{{ form.processing ? 'Saving...' : 'Publish Project' }}</span>
                        </button>
                    </div>
