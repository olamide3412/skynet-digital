...
const form = useForm({
    ...
    image_path: null, // Only included if they upload a new image
    _method: 'PUT', // Let Laravel handle form method spoofing via Inertia
});
        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg rounded-lg p-6">
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div>
                    ...
                    <input id="title" v-model="form.title" type="text" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 rounded-md shadow-sm focus:border-primary focus:ring-opacity-50" required />
                ...
                <div>
                    <label for="image_path" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                        New Project Image (Upload to replace existing image)
                    </label>
                    <div v-if="project.image_path" class="my-2">
                        <img :src="'/storage/' + project.image_path" class="h-24 w-auto rounded object-cover shadow" alt="Current Image" />
                    </div>
